...
func TestMergeSort(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			testCities := make([]City, len(cities))
			copy(testCities, cities)

			// Sort the cities
			sorted := MergeSort(testCities, tt.field, tt.direction)
...
func TestMergeSortEmpty(t *testing.T) {
	// Test empty slice
	emptyCities := []City{}
	sorted := MergeSort(emptyCities, "id", "asc")
	if len(sorted) != 0 {
		t.Errorf("Expected empty slice, got %d elements", len(sorted))
	}

	// Test single element
	singleCity := []City{{ID: 1, Name: "Test"}}
	sorted = MergeSort(singleCity, "id", "asc")
...
func TestMergeSortStability(t *testing.T) {
	...
	sorted := MergeSort(cities, "name", "asc")
